...
- 预览读取本地清单失败，或覆盖在提交安全基准前失败时，会立即取消 bootstrap 并清空活动预览；取消响应失败时保留 session 供重试，本地 session 已丢失时也会刷新服务端状态，不会静默卡在错误的建立基准状态。若已提交为 `STRICT` 后执行失败，则保留恢复包供用户回滚。
